...
REM  usage: createDB [DBNAME] [DBDRIVE] [DBDIR] [DBUSER]
...
db2 -v -t "CREATE REGULAR TABLESPACE %DBNAME% PAGESIZE 32K MANAGED BY DATABASE USING (FILE '%DBDRIVE%%DBDIR%\%DBNAME%\usr2\%DBNAME%_tbs.dbf' 128M) AUTORESIZE YES BUFFERPOOL %DBNAME%BP" ;
db2 -v -t "CREATE USER TEMPORARY TABLESPACE USERTEMP1 PAGESIZE 32K MANAGED BY DATABASE USING (FILE '%DBDRIVE%%DBDIR%\%DBNAME%\usrtmp\%DBNAME%_tmp.dbf' 64M) AUTORESIZE YES BUFFERPOOL %DBNAME%TEMPBP" ;
...
:usage
@echo usage: createDB [DBNAME] [DBDRIVE] [DBDIR] [DBUSER]
@echo where: 
...
@echo  e.g.: createDB Daphne c: \DB2 db2admin
@echo        Creates a database under c:\DB2\Daphne accessible by db2admin.
